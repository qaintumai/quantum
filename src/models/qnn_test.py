...
class QuantumNeuralNetworkModel:
    ...
    def train(self, X_train, y_train, learning_rate=0.01, epochs=3, batch_size=2):
        ...
        train_model(self.model, criterion, optimizer, train_loader, num_epochs=epochs, device=self.device)
        end_time = time.time()
...
financial_csv_path = '/Users/maxsivry/Desktop/Work/quantum/tutorials/financial.csv'
df = pd.read_csv(financial_csv_path)
df = df.drop(['Company', 'Time'], axis=1)  # Specific to the financial dataset

def preprocess_data(df, samples=120, threshold=0.55, binary=True):
    """
    Preprocess the dataframe, including optional binarization.
    """
    if binary:
        df.iloc[:, 0][df.iloc[:, 0] > threshold] = 1.0
        df.iloc[:, 0][df.iloc[:, 0] <= threshold] = 0.0
    ...
    X_test = X[split_index:samples]
    y_train = y_scaled[:split_index]
    y_test = y_scaled[split_index:samples]
...
qnn_model = QuantumNeuralNetworkModel()
qnn_model.train(X_train, y_train, learning_rate=0.01, epochs=3, batch_size=2)
qnn_model.evaluate(X_test, y_test)
