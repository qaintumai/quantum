...
import unittest
import numpy as np
from qnn.utils.weight_initializer import WeightInitializer
...
class TestWeightInitializer(unittest.TestCase):
    ...
    def test_init_weights_shape(self):
        ...
        expected_shape = (
            self.layers,
            (self.num_wires - 1) * 2 + self.num_wires +  # Interferometer 1
            self.num_wires +                            # Squeezing
            (self.num_wires - 1) * 2 + self.num_wires + # Interferometer 2
            self.num_wires +                            # Displacement
            self.num_wires                              # Kerr
        )
    ...
    def test_init_weights_values(self):
        ...
        self.assertAlmostEqual(np.std(int1_weights), self.passive_sd, delta=0.1, msg="Interferometer 1 weights std deviation out of bounds")
        self.assertAlmostEqual(np.std(s_weights), self.active_sd, delta=0.1, msg="Squeezing weights std deviation out of bounds")
        self.assertAlmostEqual(np.std(int2_weights), self.passive_sd, delta=0.1, msg="Interferometer 2 weights std deviation out of bounds")
        self.assertAlmostEqual(np.std(dr_weights), self.active_sd, delta=0.1, msg="Displacement weights std deviation out of bounds")
        self.assertAlmostEqual(np.std(k_weights), self.active_sd, delta=0.1, msg="Kerr weights std deviation out of bounds")
